...
class CaptureService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        @Suppress("DEPRECATION") val data = intent?.getParcelableExtra<Intent>(EXTRA_PROJECTION_DATA)
        if (code == Int.MIN_VALUE || data == null || !Settings.canDrawOverlays(this)) { stopSelf(); return START_NOT_STICKY }
    ...
    private fun onImage(r: ImageReader) {
        ...
        val fps = getSharedPreferences(MainActivity.PREFS,Context.MODE_PRIVATE).getInt(MainActivity.KEY_FPS,7).coerceIn(3,12)
        val now = System.currentTimeMillis()
        if (now-last < 1000L/fps || !busy.compareAndSet(false,true)) { image.close(); return }
        last = now
        try {
    ...
    private fun notification(): Notification {
        val stop = PendingIntent.getService(this,2,Intent(this,CaptureService::class.java).setAction(ACTION_STOP),PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        ...
        return Notification.Builder(this,CHANNEL).setSmallIcon(android.R.drawable.ic_menu_view).setContentTitle("Pool Trajectory Offline").setContentText("Анализ экрана активен").setContentIntent(open).setOngoing(true).addAction(android.R.drawable.ic_menu_close_clear_cancel,"Остановить",stop).build()
    }
