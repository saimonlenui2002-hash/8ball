...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        root.addView(TextView(this).apply {
            text = "Pool Trajectory Offline 4.2.1 ML"
            textSize = 25f
            setTextColor(Color.BLACK)
        })
        root.addView(TextView(this).apply {
            text = "Версия 4.2.1 больше не требует синего или зелёного полотна. Обычный ML 4.2 используется первым, а если старая проверка цвета ошибочно отвергает стол, автоматически включается цветонезависимый режим: геометрия интерфейса + штатная белая линия + ML-кольцо и короткие ветки."
            textSize = 15f
        ...
        root.addView(button("2. Запустить ML 4.2.1") { requestCapture() })
        ...
        root.addView(CheckBox(this).apply {
            text = "Диагностика: показать найденную штатную геометрию, контакт и короткие ветки"
        ...
        root.addView(TextView(this).apply {
            text = "Для теста: FPS 7, рикошеты 0. Если сейчас нет штатной линии прицеливания, статус должен показывать ожидание прицеливания, а не «игровой стол не виден». Цвет полотна может быть любым."
            textSize = 14f
    ...
    override fun onActivityResult(requestCode:Int,resultCode:Int,data:Intent?) {
        ...
        if (requestCode==RC_CAPTURE && resultCode==RESULT_OK && data!=null) {
            ...
            Toast.makeText(this,"ML 4.2.1 запущен",Toast.LENGTH_LONG).show()
        }
        refreshStatus()
    }
    ...
    private fun refreshStatus() {
        status.text=if(Settings.canDrawOverlays(this)) {
            "✓ Overlay разрешён. Версия 4.2.1 • цвет полотна не используется как обязательный признак."
